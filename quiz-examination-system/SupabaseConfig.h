...
namespace quiz_examination_system
{
    ...
    public:
        ...
        static winrt::hstring GetAuthorizationHeader()
        {
            auto &manager = SupabaseClientManager::GetInstance();
            auto jwtToken = manager.GetJwtToken();

            if (!jwtToken.empty())
            {
                return winrt::hstring(L"Bearer ") + jwtToken;
            }

            return winrt::hstring(L"Bearer ") + ANON_KEY;
        }
